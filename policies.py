...
from networks import PiNet, QNet, Mine, GausNet, QNet2, PiNet2
...
class MINEPolicy2(Policy):
    def __init__(self, scenario: Scenario, horizon: int, nsamples: int, ntrvs: int,
        ...
        self._ntrvs = ntrvs
        self._tradeoff = tradeoff
        self._pi_net = [PiNet2([ntrvs] + pi_net_sizes + [scenario.ninputs], pt.from_numpy(pi_net_cov).float()) for t in range(horizon)]
        self._q_net = [QNet2([scenario.noutputs] + q_net_sizes + [ntrvs], pt.from_numpy(q_net_cov).float(), False)] + [QNet2([scenario.noutputs + ntrvs] + q_net_sizes + [ntrvs], pt.from_numpy(q_net_cov).float()) for t in range(horizon - 1)]
        self._mine = [Mine(scenario.nstates + ntrvs, mine_params['hidden_size']) for t in range(horizon)]
    ...
    def train(self, training_iterations: int=10, qlr=0.0001, pilr=0.0001, nsamples: int=500, log: bool=True):
        ...
        writer = SummaryWriter(f'runs/tradeoff_{int(self._tradeoff)}')
