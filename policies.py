...
#     pi_log_probs = pt.zeros((horizon, batch_size))
#     q_log_probs = pt.zeros((horizon, batch_size))
#     ctg = pt.zeros((horizon, batch_size))
#     mi = pt.zeros(horizon)
#
#     mine = [mine_class() for t in range(horizon)]
#
...
#
#         for t in range(horizon):
#             ctg[t, :] = costs[t:, :].sum(axis=0)
#
#             for s in range(batch_size):
#                 #pi_log_probs[t, s] = pi_net.log_prob(inputs[:, t, s], trvs[:, t, s], t)
#                 pi_log_probs[t, s] = pi_net.log_prob(inputs[:, t, s], outputs[:, t, s], t)
#
#                 if t > 0:
...
#         opt.zero_grad()
#
#         baseline = 0 #costs.sum(axis=0).mean()
#         loss = ((pt.mul(pi_log_probs.sum(axis=0), costs.sum(axis=0) - baseline)).mean()) #+ pt.mul(q_log_probs.sum(axis=0), ctg - baseline).mean()) + tradeoff * mi_sum
#         mi_sum = mi.sum()
#
#         loss.backward()
#         opt.step()
