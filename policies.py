...
def train_mine_policy(scenario: Scenario, horizon: int, batch_size: int,
    ...
    for epoch in range(epochs):
        ...
        print(f'Mean Abs. Displacement: {pt.abs(states[0, -1, :] - states[1, -1, :]).mean().detach().item()}')
        ...
        if tag is not None:
            ...
            if log_video_every is not None and epoch % log_video_every == 0:
                ...
                best_traj_vid = pt.stack([pt.stack([outputs[:, t, best_traj_idx].view(3, 64, 64) for t in range(horizon)])])
                worst_traj_vid = pt.stack([pt.stack([outputs[:, t, worst_traj_idx].view(3, 64, 64) for t in range(horizon)])])
